...
webp_ENABLED            = pkg_config_ok("--atleast-version=0.4", "libwebp")
...
def detect_xorg_setup():
    ...
    def Xorg_suid_check():
        ...
        Xorg_args = "-dpi 96 -noreset -nolisten tcp +extension GLX +extension RANDR +extension RENDER -auth $XAUTHORITY -logfile ${HOME}/.xpra/Xorg.${DISPLAY}.log -configdir ${HOME}/.xpra/xorg.conf.d -config %s" % xorg_conf
...
if WIN32:
    ...
    if "build_ext" not in sys.argv:
        ...
        if not PYTHON3:
            ...
            add_console_exe("xpra/scripts/bug_report.py",       "bugs.ico",         "Bug_Report")
        if opengl_ENABLED:
...
if x11_ENABLED:
    ...
    cython_add(Extension("xpra.x11.bindings.window_bindings",
                ...
                **pkgconfig("x11", "xtst", "xfixes", "xcomposite", "xdamage")
                ))
...
if dec_avcodec2_ENABLED:
    ...
    if is_msvc():
        add_to_keywords(kw, 'extra_compile_args', "/Wd4996")
    else:
